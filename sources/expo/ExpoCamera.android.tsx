...
export function ExpoCamera(props: OriginalCamera['props']) {
...
function useAutoSize(
  ...
  useEffect(() => {
    const _suggestedAspectRatio = findClosestAspectRatio(
      supportedAspectRatios,
      dimensions,
    );
    const _suggestedDimensions = calculateSuggestedDimensions(
      dimensions,
      _suggestedAspectRatio,
    );

    if (!_suggestedAspectRatio || !_suggestedDimensions) {
      setSuggestedAspectRatio(null);
      setSuggestedDimensions(null);
    } else {
      setSuggestedAspectRatio(_suggestedAspectRatio);
      setSuggestedDimensions(_suggestedDimensions);
    }
...
function useSupportedAspectRatios(): [string[] | null, CameraRefCallback] {
  ...
  const ref = useCallback(
    (node: CameraRef | null) => {
      async function getSupportedAspectRatiosAsync(_node: OriginalCamera) {
        try {
          const result = await _node.getSupportedRatiosAsync();
...
function calculateSuggestedDimensions(
  ...
  try {
    ...
    const expectWidth = height / ratioNumber;
    const expectHeight = width * ratioNumber;
    return expectWidth < width
      ? { width, height: expectHeight }
      : { width: expectWidth, height };
  } catch (e) {
    // If something unexpected happens just bail out
    console.error(e);
    return null;
